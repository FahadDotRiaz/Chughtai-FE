...
const CreateUpdate = () => {
  ...
  const fieldsConfig = [
    ...
    },
    {
      ...
      showSearch: true,
      rules: [{ required: true, message: "This field is required" }],
      options: POList?.list?.map((item) => ({ value: item.id, label: item.poCode })),
    },
    // {
    // 	type: "date",
    // 	label: "Date",
  ...
  return (
    <div>
      ...
      <Form
        ...
        onFinish={onFinish}
      >
        <TitleSearchButton title="Outward Gate Pass" printBtn />
        <GenericCard>
          <FormFieldGroup
        ...
        <div className="mt-5 flex justify-between">
          ...
          <GenericButton
            type="primary"
            lable={pathname === PATH.OGP_CREATE ? "Create OGP" : "Update OGP"}
            htmlType="submit"
        ...
        <AppConfirmDialog
          ...
          title={
            <div className="text-center">
              {pathname === PATH.OGP_CREATE
                ? "OGP created successfully"
                : "OGP updated successfully"}
            </div>
          }
          footer={
            <GenericButton
