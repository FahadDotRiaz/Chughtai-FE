...
const Index = () => {
	...
	useEffect(() => {
		if (debouncedTableOptions) {
			gatePassData({ tableOptions: debouncedTableOptions, type: "Inward" });
		}
	...
	const handleDelete = async (id) => {
		...
		if (!error) {
			openNotification("success", "IGP deleted successfully");
		} else {
	...
	const hasPermission = (action) => {
		return permissions?.[MAKEYS.IGP]?.[action];
	};
				...
				<ActionDropdown
					viewOnClick={
						hasPermission(ACTION_KEYS.VIEW)
							? () => navigate(PATH.IGP_VIEW.replace(":id", cell?.getValue()))
							: undefined
					}
					editOnClick={
						hasPermission(ACTION_KEYS.EDIT)
							? () => navigate(PATH.IGP_UPDATE.replace(":id", cell?.getValue()))
							: undefined
					}
					deleteOnClick={
	...
	return isLoading ? (
		...
		<div>
			...
			<CardButtonFilterGroup
				title={{ text: "Inward Gate Pass", level: 1 }}
				button={
					hasPermission(ACTION_KEYS.ADD)
						? {
								label: "Create IGP",
								icon: <PlusOutlined />,
								onClick: () => navigate(PATH.IGP_CREATE),
						  }
						: undefined
				}
			>
			...
			<AppConfirmDialog
				...
				description="Are you sure you want to delete this IGP?"
